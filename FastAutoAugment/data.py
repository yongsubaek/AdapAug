...
def get_post_dataloader(dataset, batch, dataroot, split, split_idx, gr_id, gr_ids):
    ...
    #     total_trainset, batch_size=batch, shuffle=True if train_sampler is None else False, num_workers=4 if torch.cuda.device_count()==4 else 8, pin_memory=True,
    #     sampler=train_sampler, drop_last=True)
    validloader = torch.utils.data.DataLoader(
        total_trainset, batch_size=batch, shuffle=False, num_workers=4, pin_memory=True,
        sampler=valid_sampler, drop_last=False)
    # testloader = torch.utils.data.DataLoader(
    #     testset, batch_size=batch, shuffle=False, num_workers=4 if torch.cuda.device_count()==4 else 8, pin_memory=True,
    #     drop_last=False
    # )
    return validloader
